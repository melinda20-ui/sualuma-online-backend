...
type Filter = "all" | "flowmind" | "prestador" | "cliente";

const filters: Record<Filter, { keys: PlanKey[]; title: string; subtitle: string }> = {
  all: {
    keys: ["free", "basico", "prime", "premium", "ia_pro"],
    title: "Escolha como você quer começar",
    subtitle: "Entre gratuitamente, teste os planos pagos com segurança e evolua conforme sua operação crescer.",
  },
  flowmind: {
    keys: ["free", "basico", "premium", "ia_pro"],
    title: "Planos FlowMind",
    subtitle: "Produtividade pessoal e da empresa. Do básico ao fluxo completo com IA.",
  },
  prestador: {
    keys: ["free", "basico", "ia_pro"],
    title: "Planos para Prestadores",
    subtitle: "Divulgue seus serviços, contrate prestadores e tenha prioridade com o plano certo.",
  },
  cliente: {
    keys: ["basico", "prime", "premium", "ia_pro"],
    title: "Planos para Cliente IA",
    subtitle: "Dashboard de cliente, agentes de IA, automações e prioridade máxima.",
  },
};
...
export default function PlansContent({ filter = "all" }: Props) {
  const config = filters[filter];
  const plans = allPlans.filter((p) => config.keys.includes(p.key));
  const cols =
    plans.length <= 3 ? "xl:grid-cols-3" : plans.length === 4 ? "xl:grid-cols-4" : "xl:grid-cols-5";
  ...
  return (
    <main className="min-h-screen bg-[#050507] text-white">
      <div className="mx-auto w-full max-w-7xl px-4 py-10 md:px-6">
        ...
        <section className={`grid grid-cols-1 gap-5 ${cols}`}>
          {plans.map((plan) => (
            <article
              id={plan.key}
              key={plan.key}
